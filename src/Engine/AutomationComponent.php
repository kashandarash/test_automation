...
use Drupal\social_automation\Context\ExecutionMetadataState;
...
/**
 * Handles executable Automation components.
 */
class AutomationComponent {
   ...
   *
   * @var \Drupal\social_automation\Context\ExecutionStateInterface
   */
  protected $state;
  ...
  public static function createFromConfiguration(array $configuration) {
    ...
    $expression_manager = \Drupal::service('plugin.manager.automation_expression');
   ...
   *
   * @return \Drupal\social_automation\Context\ExecutionStateInterface
   ...
   * @return $this
   */
  public function addContextDefinition($name, CoreContextDefinition $definition) {
  ...
  public function addContextDefinitionsForEvents(array $event_names) {
    foreach ($event_names as $event_name) {
      ...
      $event_definition = \Drupal::service('plugin.manager.automation_trigger')->getDefinition($event_name);
  ...
  public function execute() {
    ...
    $automationDebugLogger = \Drupal::service('logger.channel.social_automation_debug');
    $automationDebugLogger->info('AutomationComponent: Workflow event %label fires.', [
      '%label' => $this->expression->getLabel(),
      'element' => $this->expression,
      'scope' => TRUE,
    ]);
    $this->expression->executeWithState($this->state);
    $automationDebugLogger->info('AutomationComponent: Workflow event %label has fired.', [
      '%label' => $this->expression->getLabel(),
      'element' => $this->expression,
      'scope' => FALSE,
    ]);
   ...
   *
   * @return \Drupal\social_automation\Context\ExecutionMetadataStateInterface
  ...
  public function calculateDependencies() {
    ...
    $this->addDependency('module', 'social_automation');
