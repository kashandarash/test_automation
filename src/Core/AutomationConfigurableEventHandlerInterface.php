...
/**
 * Interface for handling configurable automation events.
 *
 ...
 * which is used for triggering reaction social_automation.
 *
 * For example, the fully-qualified event name of an event for viewing an
 * article node would be "social_automation_entity_view:node--article", whereas
 * "social_automation_entity_view:node" is the base event name and "article"
 * the event suffix as returned from ::getEventNameSuffix().
 *
 ...
 * configuration.
 *
 * @see \Drupal\social_automation\Core\AutomationDefaultEventHandler
 */
interface AutomationConfigurableEventHandlerInterface extends AutomationEventHandlerInterface, ConfigurableInterface {
   ...
   *
   * @return string[]
   *   the fully-qualified event "social_automation_entity_view:node--article"
   *   should be triggered in addition to base event
   *   "social_automation_entity_view:node".
   */
   ...
   *
   * If the event is configured and a suffix is provided, the event name
   * Automation uses for the configured event is {EVENT_NAME}--{SUFFIX}.
   *
   * @return string|false
