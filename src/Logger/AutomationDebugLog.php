...
/**
 * Logger that stores Automation debug logs with the session service.
 *
 * This logger stores an array of Automation debug logs in the session under
 * the attribute named 'social_automation_debug_log'.
 */
class AutomationDebugLog implements LoggerInterface {
  use LoggerTrait;
  ...
  /**
   * Constructs a AutomationDebugLog object.
   *
  ...
  public function log($level, $message, array $context = []) {
    ...
    $localCopy = $this->session->get('social_automation_debug_log', []);
    ...
    $this->session->set('social_automation_debug_log', $localCopy);
  }
  ...
  public function getLogs() {
    return (array) $this->session->get('social_automation_debug_log');
  }
  ...
  public function clearLogs() {
    $this->session->remove('social_automation_debug_log');
  }
  ...
  public function build() {
    ...
    $build = [
      ...
      '#title' => $this->t('Automation evaluation log') . '<span class="automation-debug-open-all">-Open all-</span>',
      // @codingStandardsIgnoreEnd
      '#attributes' => ['class' => ['automation-debug-log']],
    ];
  ...
  public function renderHelper(&$line = 0) {
    ...
    while ($line < count($this->logs)) {
      if ($build && !empty($this->logs[$line]['scope'])) {
        ...
        $build[$line] = [
          '#theme' => 'social_automation_debug_log_element',
          '#starttime' => $startTime,
