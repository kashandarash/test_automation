...
/**
 * Interface for Automation UI handlers.
 *
 * Automation UI handlers define where AutomationUI instances are embedded
 * and are responsible for generating the appropriate routes.
 */
interface AutomationUiHandlerInterface extends PluginInspectionInterface {

  /**
   * {@inheritdoc}
   *
   * @return \Drupal\social_automation\Ui\AutomationUiDefinition
   *   The social_automation_ui plugin definition.
   */
  ...
  /**
   * Gets an URL for a Automation UI route.
   *
   * @param string $route_suffix
   *   The Automation UI route suffix that is appended to the base route.
   ...
   *
   * @see \Drupal\social_automation\Routing\AutomationUiRouteSubscriber::registerRoutes()
   */
  ...
  /**
   * Checks if the event has been modified and is present in the storage.
   *
   * @return bool
   *   TRUE if the workflowevent has been modified, FALSE otherwise.
   */
  public function isEdited();

  /**
   * Provides information which user at which time locked the event for editing.
   *
   * @return object
  ...
  /**
   * Renders a message if the workflowevent component is locked/modified.
   *
   * @return array
  ...
  /**
   * Validation callback that prevents editing locked workflowevent components.
   *
   * @param array $form
