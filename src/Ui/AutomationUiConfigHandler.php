...
use Drupal\social_automation\Form\EmbeddedComponentForm;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * The default handler for AutomationUi plugins that store to config.
 *
 ...
 *   \Drupal\social_automation\Ui\AutomationUiComponentProviderInterface.
 *   Required, unless config_name and config_key are provided.
 ...
 *
 * @see \Drupal\social_automation\Ui\AutomationUiDefinition::settings()
 */
class AutomationUiConfigHandler extends PluginBase implements AutomationUiHandlerInterface, ContainerFactoryPluginInterface {
  use TempStoreTrait;

  /**
   * The automation UI (plugin) definition.
   *
   * @var \Drupal\social_automation\Ui\AutomationUiDefinition
   */
   ...
   *
   * @return \Drupal\social_automation\Ui\AutomationUiComponentProviderInterface|\Drupal\Core\Config\Config
  ...
  public function getComponentLabel() {
    ...
    if ($this->getConfig() instanceof EntityInterface) {
      return (string) $this->getConfig()->label();
    }

    return (string) $this->pluginDefinition->component_type_label;
  }
  ...
  public function getComponent() {
    ...
    if ($config instanceof AutomationUiComponentProviderInterface) {
      return $config->getComponent();
    }
    else {
      $configuration = $config->get($this->pluginDefinition->settings['config_key']);
      return AutomationComponent::createFromConfiguration($configuration);
    }
  }
   ...
   * {@inheritdoc}
   */
  public function updateComponent(AutomationComponent $component) {
    $config = $this->getConfig();
    if ($config instanceof AutomationUiComponentProviderInterface) {
